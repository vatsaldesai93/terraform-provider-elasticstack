...
- [ ] 1.2 Replace the `--packages="./..."` argument in the `testacc` recipe with `--packages="$(shell go list ./... | awk 'NR % $(ACCTEST_TOTAL_SHARDS) == $(ACCTEST_SHARD_INDEX)')"`.
...
- [ ] 1.4 Verify that `make testacc ACCTEST_TOTAL_SHARDS=2 ACCTEST_SHARD_INDEX=0` and `ACCTEST_SHARD_INDEX=1` together cover all packages: `diff <(go list ./... | sort) <(cat <(go list ./... | awk 'NR%2==0') <(go list ./... | awk 'NR%2==1') | sort)` should produce no output.
